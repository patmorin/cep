...
For $i\in\{1,\ldots,p\}$, an \defin{$i$-exit edge} is an edge $e$ of $U_i$ with exactly one endpoint in $U^-_i$.
...
\begin{clm}\label{grow_in_forests}
  ...
  \begin{compactenum}[(i)]
    ...
    \item $G[B_G(V(P_0),d)]=F_0[B_{F_0}(V(P_0),d)]$; \pat{Maybe just show $B_G(V(P_0),d)=B_{F_0}(V(P_0),d)$?}; \label[p]{gif_zero}
    and \item $G[B_G(V(P_1),d)]=F_i[B_{F_i}(V(P_1),d)]$ and $G[B_G(V(P_2),d)]=F_j[B_{F_j}(V(P_2),d)]$; \pat{Similar comment to the previous.}\label[p]{gif_ij}
  \end{compactenum}
\end{clm}

...
\begin{frame}
  ...
  \only<+>{\textbf{Lemma:} Any cycle in $G$ is \textcolor{blue}{short} or \textcolor{blue}{close to} a cycle in $\mathcal{C}$}%
  \only<+>{Iteratively hit each short cycle with a single ball}%
  \only<+>{Iteratively hit each short cycle with a single ball}%
  \only<+>{Every (unhit) cycle in $G$ is close to a cycle in $\mathcal{C}$}%

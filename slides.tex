...
\begin{frame}
  ...
  \only<1>{\includegraphics[scale=1.2,page=22]{figs/animation}}%
  \only<2>{\includegraphics[scale=1.2,page=21]{figs/animation}}
\end{frame}
...
\begin{frame}
  ...
  \only<5-8>{\textbf{Lemma$^\star\colon$} For each $i\in\{1,\ldots,p\}$,  $B(C_i,6d)-B(X_i,13d)$ is acyclic, for some $X_i$ with $|X_i|\le f'(k)$}%
  ...
  \only<13-14>{\textbf{Lemma$^\star\colon$} For each distinct $i,j\in\{1,\ldots,p\}$, $B(C_i,6d)\cap B(C_j,6d)\subseteq B(X_{\{i,j\}},13d)$, for some $X_{\{i,j\}}$ with $|X_{\{i,j\}}|\le f'(k)$}
...
\begin{frame}
  ...
  \end{enumerate}
  \vspace{3ex}
  \uncover<2->{In Case~2, $B(X,4d)$ hits every remaining cycle in $G$}
  \vspace{2ex}
\end{frame}

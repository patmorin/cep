...
\begin{lem}
Let $\ell:\mathbb{N}^2\to\mathbb{N}$ be defined as $\ell(k,c):=(\prod_{i=1}^{p-1}2^{i!})k^{p!}$.
...
either
   \begin{tightenum}%[(a)]
     ...
     \item for every $i\in[c]$ there exist $X_i \subseteq V(F_i)$ with
     $|X_i|\leq \ell(k,c)$ and such that for each $i\in[c]$ and each $(A_1,\ldots,A_c)\in\mathcal{A}$,
...
we assume that for all $j\in[k]$, $i\in[c]$, $A\in\mathcal{A}_j$,
we have $V(A_j)\cap V(F_i)\neq\emptyset$.
Indeed, otherwise for all $j$, $i$, $A$ violating this rule, we introduce a new vertex in $F_i$ and make $A$ be the graph that contains only this new vertex.  By introducing a new vertex $r_i$ adjacent to the root of each component of $F_i$, we may further assume that $F_i$ is a tree rooted at $r_i$.

First we claim that it is enough to prove the statement with an additional assumption that for each $i\in[c]$, $F_i$ is a binary forest,\footnote{A \defin{binary tree} is a rooted tree in which each vertex has at most two children. The root of a binary tree is not considered to be a leaf, even if it has no children.  A \defin{binary forest} is forest of binary trees.} by the following argument.
For each $i\in[c]$, and each vertex $v$ in $F_i$ with children $u_1,\ldots,u_\beta$, define $T_i(v)$ to be a binary tree rooted at $v$ and whose leaves are $u_1,\ldots,u_\beta$, and let $T^-_i(v):=T_i(v)-\{u_1,\ldots,u_\beta\}$.  Let $F_i':=\bigcup_{v\in V(F_i)} T_i(v)$.  For each $A\in\mathcal{A}$, define $A':=(A'_1,\ldots,A'_c)$ where $A'_i:=F_i'[\bigcup_{v\in V(A)} V(T^-_i(v))]$ for each $i\in [c]$ and define $\mathcal{A}':=\{A':A\in\mathcal{A}\}$.  Then $\mathcal{A}'$ is a collection of  $(F_i')_{i\in[c]}$-tuples. Observe that for all $A,B\in\mathcal{A}$,
(1) $A$, $B$ are independent if and only if $A'$, $B'$ are independent;
(2) if $A_i'$ contains a vertex in $T_i^-(v)$ for some $i\in[c]$ and $v\in V(F_i)$, then $A_i$ contains $v$.  Now apply the lemma to $(F'_i)_{i\in[c]}$ and $\mathcal{A}':=\{A'\mid A\in\mathcal{A}\}$.  If this results in an independent set $I'\subseteq\mathcal{A}'$ of size $k$, then (1) ensures that $I:=\{A\mid A'\in I'\}$ satisfies \cref{item:hungarian-support:independent-set}.  If this results in sets $X_1',\ldots,X_c'$, then define $X_i:=\{v\in V(F_i): V(T_i^-(v))\cap X_i'\neq\emptyset\}$ for each $i\in[c]$.  Then (2) ensures that the sets $X_1,\ldots,X_c$ satisfy \cref{item:hungarian-support:hitting-set}.  From this point on, we assume that $F_i$ is a binary forest, for each $i\in[c]$.
...
let $\sigma$
% =(v_1,\ldots,v_n)$
be a topological ordering of vertices of $F_1$ and, for $v,w\in V(F_1)$ use the notation $v<w$ if and only if $v$ precedes $w$ in $\sigma$.

Consider the following iterative process, that constructs a sequence $v_1<\cdots<v_m$ of vertices in $V(F_1)$ and a sequence $\mathcal{D}_1,\ldots,\mathcal{D}_m$ of $(F_i)_{i\in\{2,\ldots,c\}}$-tuples.
 % $(\alpha_1,\ldots,\alpha_m)$ of increasing integers from $[n]$.
...
Suppose that $v_1,\ldots,v_{\ell-1}$ and $\mathcal{D}_1,\ldots,\mathcal{D}_{\ell-1}$ has already been defined, for some $\ell\ge 1$ and define
%
...
\begin{align}
F_{(\ell-1)}&=F_1-\set{v_1,\ldots,v_{\ell-1}},\notag
%\intertext{and for every $j\in\set{\alpha_{\ell-1}+1,\ldots,n}$ we let}
\intertext{and for each $v\in V(F_1)$ with $v>v_{\ell-1}$, we let}
\mathcal{B}_{\ell-1}(v)&=\set{A\in \mathcal{A}\mid \pi_1(A)\subseteq F_{(\ell-1)}(v)},\label{eq:Bv}\\
\mathcal{C}_{\ell-1}(v)&=\set{(\pi_2(A),\ldots,\pi_c(A))\mid A\in\mathcal{B}_{\ell-1}(v)}.\notag
\end{align}
If there exists $v\in V(F_1)$ such $v>v_{\ell}$ and $\mathcal{C}(v)$ contains an independent set of size $k^{c-1}$, then we define $v_\ell$ to the minimum such $v$ (with respect to the ordering $\sigma$).  At the same time, we define the set $\mathcal{D}_\ell$ to be an independent set of size $k^{c-1}$ contained in $\mathcal{C}_{\ell-1}(v_\ell)$.
If no such vertex $v$ exists, then
we set $m=\ell-1$ and stop the process.

\pat{Maybe define $\mathcal{E}_1,\ldots,\mathcal{E}_m$ later...}
...
(2) $m<k$, in which case we show that there exist $X_i$ for each $i\in[c]$ such that \eqref{item:hungarian-support:hitting-set} holds.

Suppose first that $m\geq k$ so that $v_1,\ldots,v_k$ and $\mathcal{D}_1,\ldots,\mathcal{D}_k$ are well-defined.  For each $\ell\in[k]$, define $\mathcal{B}_{\ell}:=\mathcal{B}_{\ell-1}(v_\ell)$. Consider $\alpha,\beta\in[k]$ with $\alpha<\beta$. Since $v_{\alpha} < v_{\beta}$, we conclude that $v_{\beta} \not\in F_1(v_{\alpha})$. In particular, $v_{\beta} \not\in F_{(\alpha-1)}(v_{\alpha})$. Since $\alpha<\beta$ we have $v_{\alpha}\not\in V(F_{(\beta-1)})$. We conclude that
\begin{equation}
  V(F_{(\alpha-1)}(v_{\alpha})) \cap V(F_{(\beta-1)}(v_{\beta})) = \emptyset.
  \label{eq:F-i-1-disjoint-from-F-j-1}
\end{equation}
Note that by~\eqref{eq:Bv} for all $A\in \mathcal{B}_{\alpha}$, $B\in\mathcal{B}_{\beta}$, we have that
$\pi_1(A)\subseteq F_{(\alpha-1)}(v_{\alpha})$,  $\pi_1(B)\subseteq F_{(\beta-1)}(v_{\beta})$.
...
For each $\ell\in[k]$, select some $A_\ell\in\mathcal{A}$ such that $(\pi_2(A_\ell),\ldots,\pi_c(A_\ell))=D_\ell$.   We now show that $\set{A_1,\ldots,A_k}$ is an independent set of $(F_i)_{i\in[c]}$-tuples.
To this end, let $A:=A_\alpha$ and $B:=B_\beta$ for some $\alpha,\beta\in[k]$ with $\alpha < \beta$.  We must show that $V(\pi_i(A))\cap V(\pi_i(B))=\emptyset$, for each $i\in[c]$.
Since $A\in\mathcal{B}_{\alpha}$ and $B\in\mathcal{B}_{\beta}$, by~\cref{eq:B-disjoin-from-B'} we have that $V(\pi_1(A))\cap V(\pi_1(B))=\emptyset$.
...
Now we suppose that $m<k$.
For each $\ell\in\{1,\ldots,m\}$, let $U_{\ell}$ be the set of (at most two) children of $v_\ell$ in $F_1$ and define $\mathcal{E}_{\ell}:=\bigcup_{u\in U_\ell}\mathcal{C}_{\ell-1}(u)$.  By the minimality of $v_\ell$, $\mathcal{C}_{\ell-1}(u)$ does not contain an independent set of size $k^{c-1}$ for each $\ell\in[m]$ and $u\in U_{\ell}$.  Therefore $\mathcal{E}_{\ell}$ does not contain an independent set of size $2k^{c-1}$ for each $\ell\in[m]$.  Next, recall that $F_1$ is a tree with root $r_1$.  If $r_1 = v_m$, then define $\mathcal{E}_{m+1}:=\emptyset$. Otherwise, define $\mathcal{E}_{m+1}:=\mathcal{C}_{m}(r_1)$.  In the latter case, since the process for constructing $v_1,\ldots,v_m$ stopped at iteration $m$, $\mathcal{E}_{m+1}=\mathcal{C}_{m}(r_1)$ does not contain an independent set of size $k^{c-1}$. Therefore $\mathcal{A'}:=\bigcup_{\ell\in[m+1]}\mathcal{E}_\ell$ does not contain an independent set of size $(m+1)2k^{c-1} \le 2k^{c}$.

Observe that $\mathcal{A}'$ is a collection of $(F_i)_{i\in\{2,\ldots,c\}}$-tuples.  Apply the inductive hypothesis with $c':=c-1$ and $k':=2k^{c}$ to the collection $\mathcal{A}'$ of $(F_i)_{i\in\{2,\ldots,c\}}$-tuples.  Since $\mathcal{A}'$ does not contain an independent set of size $k'$, the result of applying the inductive hypothesis is outcome \cref{item:hungarian-support:hitting-set}.  That is, we obtain a sequence of sets $X_2,\ldots,X_c$ with $X_i\subseteq V(F_i)$ and $|X_i|\le\ell(2k^{c},c-1)$, for each $i\in\{2,\ldots,c\}$ and such that, for each $(A_2,\ldots,A_c)\in\mathcal{A}'$, there exists some $i\in\{2,\ldots,c\}$ such that $V(A_i)\cap X_i\neq\emptyset$.


Let $X_1:=\{v_1,\ldots,v_m\}$.  We claim that $X_1,\ldots,X_c$ satisfies \cref{item:hungarian-support:hitting-set}.

First we bound the size of $X_i$ for each $i\in[c]$.  By definition, $|X_1|=m\le k< 2^{c-1}k^{c}$ since $c>1>0$.  By the inductive hypothesis,
\[\textstyle
  |X_i|
   \le \left(\prod_{i=1}^{c-2}2^{i!}\right)(k')^{(c-1)!}
   = \left(\prod_{i=1}^{c-2}2^{i!}\right)(2k^c)^{(c-1)!}
   = \left(\prod_{i=1}^{c-1}2^{i!}\right)k^{c!} ,
\]
for each $i\in\{2,\ldots,c\}$.

Next we show that, for each $(A_1,\ldots,A_c)\in\mathcal{A}$, there exists $i\in[c]$ such that $V(A_i)\cap X_i\neq\emptyset$.  If $(A_2,\ldots,A_c)\in \mathcal{A}'$ then, by the inductive hypothesis, $V(A_i)\cap X_i\neq\emptyset$ for some $i\in\{2,\ldots,c\}$.  If $(A_2,\ldots,A_c)\notin\mathcal{A}'$, then this is because there is no $\ell\in[m+1]$ such that $(A_2,\ldots,A_c)\in\mathcal{E}_{\ell}$.  Consider the vertex $v$ of $A_1$ closest to the root of $F_1$.  If $v > v_m$, then $v_m$, then $A_1\not$
%
%

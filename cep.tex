...
We say that $\sigma$ is a \defin{topological sorting} of $F$ if for each $v\in V(F)$, every vertex of $F(v)$ precedes $v$ in $\sigma$.

Let $c$ be a positive integer and 
let $F_i$ be a forest for each $i\in[c]$. \pat{I guess $F_1,\ldots,F_c$ are not necessarily pairwise vertex-disjoint?}
\piotr{Right.}
